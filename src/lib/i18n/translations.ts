/**
 * Bilingual (English / Arabic) UI strings.
 * Lightweight inline i18n — no next-intl complexity needed for two languages.
 * English is the DEFAULT locale (since v0.3).
 */

export type Locale = "ar" | "en";

export const LOCALE_STORAGE_KEY = "icd10_locale_v2";

export const translations = {
  en: {
    dir: "ltr" as const,
    app_title: "ICD-10-CM AI Coding Agent",
    app_subtitle: "Convert clinical text into ICD-10-CM codes ranked dynamically (Primary / Secondary / Supplemental)",
    language_toggle: "العربية",
    model_label: "LLM Model",
    model_help: "Choose the model used for coding. Auto tries free online models then falls back to the Smart Offline Coder.",
    input_label: "Clinical Note",
    input_placeholder: "Type or paste the clinical note here. Example: Patient came to ER with cat scratch on his right lower leg today, controlled type 2 diabetes.",
    sample_cases: "Sample cases",
    analyze_button: "Analyze & Code",
    analyzing: "Analyzing…",
    clear_button: "Clear",
    results_title: "Coding Results",
    no_results: "Enter a clinical note and click Analyze to see results here.",
    primary: "Principal Diagnosis",
    secondary: "Secondary Diagnoses",
    secondary_hint: "All co-existing conditions that affect care during this encounter — chronic AND acute (per UHDDS / ICD-10-CM Official Guidelines Section III). Examples: diabetes, hypertension, CKD, anemia, dehydration.",
    tertiary: "Supplemental Codes",
    tertiary_hint: "External cause codes (V/W/X/Y) describing HOW an injury happened, place of occurrence, activity, and other supporting codes. Per Chapter 20 rules these are reported LAST — never as the principal diagnosis.",
    confidence: "Confidence",
    rationale: "Clinical Rationale",
    laterality: "Laterality",
    acuity: "Acuity",
    seventh_char: "7th Char",
    validation_title: "Validation Alerts",
    no_validation: "No alerts — all codes passed validation.",
    errors: "errors",
    warnings: "warnings",
    info: "info",
    rag_context_title: "RAG Context (retrieved codes)",
    rag_empty: "No codes retrieved from RAG.",
    nlm_status_ok: "NLM API: online",
    nlm_status_fail: "NLM API: unreachable — using local DB",
    export_button: "Export PDF",
    export_help: "Open the print dialog then choose \"Save as PDF\"",
    latency: "Latency",
    ms: "ms",
    entities_title: "Extracted Clinical Entities (NER)",
    raw_json: "Raw JSON",
    summary: "Summary",
    level_primary: "Primary",
    level_secondary: "Secondary",
    level_tertiary: "Supplemental",
    not_applicable: "N/A",
    right: "Right",
    left: "Left",
    bilateral: "Bilateral",
    unspecified: "Unspecified",
    acute: "Acute",
    chronic: "Chronic",
    acute_on_chronic: "Acute on chronic",
    char_a: "A — initial encounter",
    char_b: "B — initial, open fx (Gustilo I/II)",
    char_c: "C — initial, open fx (Gustilo III)",
    char_d: "D — subsequent",
    char_s: "S — sequela",
    char_g: "G — delayed healing",
    char_k: "K — nonunion",
    char_p: "P — malunion",
    char_not_required: "not required",
    char_missing: "MISSING (!)",
    footer: "Experimental coding agent for educational use — do not use for diagnosis or billing without human review.",
    snomed_attribution: "SNOMED CT® is licensed material — embedded here as a validated 273-concept demo subset; production deployment requires an appropriate SNOMED CT license (SNOMED International affiliate or US NLM UMLS).",
    copy_json: "Copy JSON",
    copied: "Copied",
    copy_code: "Copy code",
    search_title: "ICD-10-CM Lookup",
    search_placeholder: "Search 74,000+ codes — e.g. \"epigastric pain\" or \"S82.2\"…",
    search_no_results: "No matching codes.",
    search_full_db_on: "Full offline DB",
    search_curated_only: "Curated DB",
    search_hint_full: "Searching the full ICD-10-CM database offline.",
    search_hint_curated: "Full database not loaded yet — searching the curated set.",
    search_hint_nlm: "NLM cross-check when online.",
    offline_db_title: "Offline Database",
    offline_db_desc: "The complete ICD-10-CM code set (~74,000 codes) stored in your browser. Works fully offline.",
    offline_db_status: "Status",
    offline_db_ready: "Ready",
    offline_db_loading: "Loading…",
    offline_db_error: "Error",
    offline_db_not_loaded: "Not loaded",
    offline_db_codes: "Codes",
    offline_db_source: "Source",
    offline_db_source_bundled: "Bundled snapshot (this release)",
    offline_db_source_live: "Live NLM refresh",
    offline_db_version: "Data version",
    offline_db_load_btn: "Load full database",
    offline_db_refresh_btn: "Refresh from NLM",
    offline_db_refresh_hint: "Pull the latest code set directly from the NLM Clinical Tables API (~1–2 min).",
    offline_db_refreshing: "Downloading from NLM…",
    offline_db_clear: "Clear",
    offline_db_footnote: "Refresh pulls all codes from NLM and stores them in your browser — useful when the annual code update ships before a new app release.",
    offline_db_billable_short: "billable",
    code_verified: "Verified in ICD-10-CM",
    code_category_only: "Category code — needs full specificity",
    code_not_found: "Not found in ICD-10-CM — verify manually",
    welcome: "Welcome",
    login_title: "Sign In",
    login_subtitle: "Enter your credentials to access the coding agent",
    username_label: "Username",
    password_label: "Password",
    login_button: "Sign In",
    logging_in: "Signing in…",
    logout_button: "Sign Out",
    login_error: "Invalid credentials. Please try again.",
    demo_hint: "Demo: username omar / password omar123",
    signed_in_as: "Current user",
    session_protected: "This page is protected. Sign in to continue.",
    // --- Theme & display settings ---
    appearance: "Appearance",
    theme_label: "Color theme",
    theme_emerald: "Emerald (default)",
    theme_ocean: "Ocean",
    theme_violet: "Violet",
    theme_rose: "Rose",
    theme_amber: "Amber",
    theme_mono: "Slate Mono",
    mode_light: "Light",
    mode_dark: "Dark",
    mode_system: "System",
    font_size_label: "Text size",
    font_small: "Small",
    font_normal: "Normal",
    font_large: "Large",
    font_xl: "Extra large",
    appearance_reset: "Reset",
    // --- Mobile ---
    codes_summary: "Codes",
    copy_all_codes: "Copy all codes",
    offline_ready: "Works offline",
    error_title: "Analysis failed",
    try_prefix: "Try:",
    try_auto: "Switch to Auto (online + offline fallback)",
    try_offline: "Switch to Smart Offline Coder",
    try_retry: "Retry",
    how_get_keys: "How to get free API keys?",
    free_badge: "FREE",
    ready_badge: "ready",
  },
  ar: {
    dir: "rtl" as const,
    app_title: "وكيل ترميز ICD-10-CM الذكي",
    app_subtitle: "حوّل النص الإكلينيكي إلى رموز ICD-10-CM مرتبة ديناميكياً (رئيسي / ثانوي / تكميلي)",
    language_toggle: "English",
    model_label: "النموذج اللغوي",
    model_help: "اختر النموذج المستخدم في الترميز. الوضع التلقائي يجرب النماذج المجانية ثم يتحول إلى المُرمّز الذكي دون اتصال.",
    input_label: "النص الإكلينيكي (Clinical Note)",
    input_placeholder: "اكتب أو الصق النص الإكلينيكي هنا. مثال: Patient came to ER with cat scratch on his right lower leg today, controlled type 2 diabetes.",
    sample_cases: "حالات نموذجية",
    analyze_button: "تحليل وترميز",
    analyzing: "جارٍ التحليل…",
    clear_button: "مسح",
    results_title: "نتائج الترميز",
    no_results: "أدخل نصاً إكلينيكياً واضغط تحليل لعرض النتائج هنا.",
    primary: "التشخيص الرئيسي",
    secondary: "التشخيصات الثانوية",
    secondary_hint: "جميع الحالات المصاحبة المؤثرة على خطة العلاج خلال هذه الزيارة — المزمنة والحادة (وفق تعريف UHDDS ودليل ICD-10-CM الرسمي القسم الثالث). أمثلة: السكري، ضغط الدم، القصور الكلوي، فقر الدم، الجفاف.",
    tertiary: "الرموز التكميلية",
    tertiary_hint: "رموز الأسباب الخارجية (V/W/X/Y) التي تصف كيف حدثت الإصابة، ومكان الحدوث، والنشاط، ورموز مساندة أخرى. وفق قواعد الفصل 20 تُبلَّغ هذه الرموز في النهاية — وليست رئيسية أبداً.",
    confidence: "درجة الثقة",
    rationale: "المبرر الإكلينيكي",
    laterality: "الجانب",
    acuity: "الحدة",
    seventh_char: "الحرف السابع",
    validation_title: "تنبيهات التحقق",
    no_validation: "لا توجد تنبيهات. جميع الرموز اجتازت التحقق.",
    errors: "أخطاء",
    warnings: "تحذيرات",
    info: "معلومات",
    rag_context_title: "سياق RAG (الرموز المسترجعة)",
    rag_empty: "لم يُسترجع أي رمز من RAG.",
    nlm_status_ok: "NLM API: متصل",
    nlm_status_fail: "NLM API: غير متاح — استخدام قاعدة البيانات المحلية",
    export_button: "تصدير PDF",
    export_help: "افتح نافذة الطباعة ثم اختر \"حفظ كـ PDF\"",
    latency: "زمن الاستجابة",
    ms: "مللي ثانية",
    entities_title: "الكيانات الطبية المستخرجة (NER)",
    raw_json: "JSON الخام",
    summary: "الملخص",
    level_primary: "رئيسي",
    level_secondary: "ثانوي",
    level_tertiary: "تكميلي",
    not_applicable: "غير مطبّق",
    right: "يمين",
    left: "يسار",
    bilateral: "ثنائي",
    unspecified: "غير محدد",
    acute: "حاد",
    chronic: "مزمن",
    acute_on_chronic: "حاد على مزمن",
    char_a: "A — زيارة أولى",
    char_b: "B — أولى، كسر مفتوح (درجة I/II)",
    char_c: "C — أولى، كسر مفتوح (درجة III)",
    char_d: "D — متابعة",
    char_s: "S — مضاعفات متأخرة",
    char_g: "G — التئام متأخر",
    char_k: "K — عدم التحام",
    char_p: "P — الئام خاطئ",
    char_not_required: "غير مطلوب",
    char_missing: "مفقود (!)",
    footer: "وكيل ترميز تجريبي للأغراض التعليمية — لا يُستخدم للتشخيص أو الفوترة دون مراجعة بشرية.",
    snomed_attribution: "SNOMED CT® مادة مرخَّصة — مضمَّنة هنا كمجموعة فرعية تجريبية مُتحقَّق منها من 273 مفهوماً؛ يتطلب النشر الإنتاجي ترخيص SNOMED CT المناسب (انتساب SNOMED International أو ترخيص UMLS الأمريكي).",
    copy_json: "نسخ JSON",
    copied: "تم النسخ",
    copy_code: "نسخ الرمز",
    search_title: "البحث في ICD-10-CM",
    search_placeholder: "ابحث في أكثر من 74,000 رمز — مثال: \"ألم في المعدة\" أو \"S82.2\"…",
    search_no_results: "لا توجد رموز مطابقة.",
    search_full_db_on: "قاعدة البيانات الكاملة",
    search_curated_only: "قاعدة مختارة",
    search_hint_full: "البحث في قاعدة بيانات ICD-10-CM الكاملة دون اتصال.",
    search_hint_curated: "لم تُحمَّل قاعدة البيانات الكاملة بعد — البحث في المجموعة المختارة.",
    search_hint_nlm: "مقارنة مع NLM عند الاتصال.",
    offline_db_title: "قاعدة البيانات دون اتصال",
    offline_db_desc: "مجموعة أكواد ICD-10-CM الكاملة (~74,000 رمز) مخزّنة في متصفحك. تعمل دون اتصال بالكامل.",
    offline_db_status: "الحالة",
    offline_db_ready: "جاهزة",
    offline_db_loading: "جارٍ التحميل…",
    offline_db_error: "خطأ",
    offline_db_not_loaded: "غير محمّلة",
    offline_db_codes: "الأكواد",
    offline_db_source: "المصدر",
    offline_db_source_bundled: "نسخة مرفقة (مع هذا الإصدار)",
    offline_db_source_live: "تحديث مباشر من NLM",
    offline_db_version: "إصدار البيانات",
    offline_db_load_btn: "تحميل قاعدة البيانات الكاملة",
    offline_db_refresh_btn: "تحديث من NLM",
    offline_db_refresh_hint: "سحب أحدث مجموعة أكواد مباشرة من NLM Clinical Tables API (~1–2 دقيقة).",
    offline_db_refreshing: "جارٍ التنزيل من NLM…",
    offline_db_clear: "مسح",
    offline_db_footnote: "يسحب التحديث جميع الأكواد من NLM ويخزّنها في متصفحك — مفيد عند صدور التحديث السنوي للأكواد قبل إصدار جديد من التطبيق.",
    offline_db_billable_short: "قابل للفوترة",
    code_verified: "مُتحقق منه في ICD-10-CM",
    code_category_only: "رمز فئة — يحتاج تحديدًا كاملًا",
    code_not_found: "غير موجود في ICD-10-CM — تحقق يدويًا",
    welcome: "مرحباً",
    login_title: "تسجيل الدخول",
    login_subtitle: "أدخل بياناتك للوصول إلى وكيل الترميز",
    username_label: "اسم المستخدم",
    password_label: "كلمة المرور",
    login_button: "تسجيل الدخول",
    logging_in: "جارٍ الدخول…",
    logout_button: "تسجيل الخروج",
    login_error: "بيانات الدخول غير صحيحة. حاول مرة أخرى.",
    demo_hint: "تجريبي: المستخدم omar / كلمة المرور omar123",
    signed_in_as: "المستخدم الحالي",
    session_protected: "هذه الصفحة محمية. سجّل الدخول للمتابعة.",
    // --- Theme & display settings ---
    appearance: "المظهر",
    theme_label: "نظام الألوان",
    theme_emerald: "زمردي (افتراضي)",
    theme_ocean: "محيطي",
    theme_violet: "بنفسجي",
    theme_rose: "وردي",
    theme_amber: "عنبري",
    theme_mono: "رمادي موحد",
    mode_light: "فاتح",
    mode_dark: "داكن",
    mode_system: "النظام",
    font_size_label: "حجم النص",
    font_small: "صغير",
    font_normal: "عادي",
    font_large: "كبير",
    font_xl: "كبير جداً",
    appearance_reset: "استعادة الافتراضي",
    // --- Mobile ---
    codes_summary: "الرموز",
    copy_all_codes: "نسخ جميع الرموز",
    offline_ready: "يعمل دون اتصال",
    error_title: "خطأ في التحليل",
    try_prefix: "جرّب:",
    try_auto: "تبديل إلى Auto (متصل + احتياطي دون اتصال)",
    try_offline: "تبديل إلى المُرمّز الذكي (دون اتصال)",
    try_retry: "إعادة المحاولة",
    how_get_keys: "كيف تحصل على مفاتيح API مجانية؟",
    free_badge: "مجاني",
    ready_badge: "جاهز",
  },
} as const;

export type TranslationKey = keyof typeof translations["en"];

export function t(locale: Locale, key: TranslationKey): string {
  return translations[locale][key];
}

export function detectDefaultLocale(): Locale {
  if (typeof window === "undefined") return "en";
  try {
    const saved = window.localStorage.getItem(LOCALE_STORAGE_KEY);
    if (saved === "ar" || saved === "en") return saved;
  } catch {
    // localStorage may be blocked — ignore
  }
  return "en";
}

export const SAMPLE_CASES: { label_en: string; label_ar: string; text: string }[] = [
  {
    label_en: "Cat scratch + DM2",
    label_ar: "خدش قط + سكري",
    text: "Patient came to ER with cat scratch on his right lower leg today, controlled type 2 diabetes.",
  },
  {
    label_en: "DM2 foot ulcer",
    label_ar: "سكري + قرحة قدم",
    text: "55-year-old male with long-standing type 2 diabetes presents with a non-healing ulcer on the right heel for 6 weeks. No gangrene. Hypertension well-controlled on lisinopril.",
  },
  {
    label_en: "COPD exacerbation",
    label_ar: "انتكاس COPD",
    text: "62-year-old female with COPD presenting with increased dyspnea, purulent sputum, and wheeze for 3 days. No fever. Home nebulizer ineffective.",
  },
  {
    label_en: "Dog bite, left hand",
    label_ar: "عضة كلب، اليد اليسرى",
    text: "Patient presents with dog bite on left hand sustained this morning. Wound is clean, no tendon involvement. Td vaccine up to date.",
  },
  {
    label_en: "HTN + CKD follow-up",
    label_ar: "متابعة ضغط + قصور كلوي",
    text: "Follow-up visit for chronic kidney disease stage 3 and essential hypertension. Creatinine stable. No edema. Patient also has hyperlipidemia.",
  },
  {
    label_en: "Fall + fractures (bilateral OA, anemia)",
    label_ar: "سقوط + كسور (خشونة وفقر دم)",
    text: "78-year-old woman tripped on the stairs at home and fell onto her left side this morning. Left wrist pain and deformity, X-ray shows distal radius fracture. Also known bilateral knee osteoarthritis and chronic anemia. No head strike, no loss of consciousness.",
  },
  {
    label_en: "Open fracture (Gustilo II)",
    label_ar: "كسر مفتوح (غوستيلو II)",
    text: "41-year-old construction worker presents after falling from a ladder with an open fracture of the right tibia, Gustilo type II. Wound 4 cm, no vascular deficit. Tetanus given.",
  },
  {
    label_en: "Rule-out + history coding",
    label_ar: "تشخيص مشكوك فيه + سابقة",
    text: "68-year-old man for medication review. History of stroke with no residual deficits, family history of diabetes. Cough for 3 days, probable pneumonia. Denies fever. On aspirin.",
  },
];
