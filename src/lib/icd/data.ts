...
export const CODE_FIRST_RULES: CodeFirstRule[] = [
  ...
  // ------------------------------------------------------------------
  // Sprint 2 (idea H): expanded official Code-First / Use-Additional
  // sequencing rules from the ICD-10-CM Tabular List notes.
  // ------------------------------------------------------------------
  {
    rule_id: "HTN_WITH_HEART_FAILURE",
    description_en: "Hypertensive heart disease with heart failure uses I11.0 (hypertensive heart disease with heart failure) PLUS a heart failure code I50.-. The heart failure code is sequenced AFTER the combination code.",
    description_ar: "مرض القلب بارتفاع الضغط مع فشل القلب يستخدم I11.0 (مرض قلب بارتفاع الضغط مع فشل القلب) بالإضافة إلى رمز فشل القلب I50.-، ويُسرد رمز فشل القلب بعد الرمز المركب.",
    trigger_codes: ["I11.0"],
    companion_codes: ["I50"],
    pattern_keywords: ["hypertensive heart", "hypertension with heart failure", "htn with chf", "hypertensive with hf"],
  },
  {
    rule_id: "HTN_CKD_WITH_HEART_FAILURE",
    description_en: "Hypertensive CKD with heart failure requires I13.- (hypertensive heart and CKD) PLUS N18.- and I50.-. I13 assumes the causal relationship between hypertension and both CKD and heart failure.",
    description_ar: "القصور الكلوي بارتفاع الضغط مع فشل القلب يتطلب I13.- (مرض قلب وكلوي بارتفاع الضغط) بالإضافة إلى N18.- و I50.-.",
    trigger_codes: ["I13"],
    companion_codes: ["N18", "I50"],
    pattern_keywords: ["hypertensive heart and ckd", "hypertension ckd heart failure"],
  },
  {
    rule_id: "DIABETIC_CKD_CODE_FIRST",
    description_en: "Diabetic CKD (E11.22 / E10.22 / E13.22) requires the CKD stage code N18.- coded FIRST (or as the underlying condition), with the diabetes combination code also present.",
    description_ar: "القصور الكلوي السكري (E11.22) يتطلب رمز مرحلة القصور الكلوي N18.- ويُرمز أولاً، مع وجود رمز السكري المركب.",
    trigger_codes: ["E11.22", "E10.22", "E13.22"],
    companion_codes: ["N18.9", "N18.6", "N18.5", "N18.4", "N18.32", "N18.31", "N18.3", "N18.2", "N18.1"],
    code_first_codes: ["N18"],
    pattern_keywords: ["diabetic ckd", "diabetic chronic kidney", "diabetes with ckd", "diabetic nephropathy with ckd"],
  },
  {
  ...
  {
    rule_id: "DIABETIC_CATARACT_CODE_FIRST",
    description_en: "Diabetic cataract codes (E11.36-, E10.36-, E13.36-) require the specific cataract code (H25.- to H28.-) coded first.",
    description_ar: "رموز إعتام العدسة السكري (E11.36-) تتطلب رمز الإعتام النوعي (H25.- إلى H28.-) ويُرمز أولاً.",
    trigger_codes: ["E11.36", "E10.36", "E13.36"],
    companion_codes: ["H25", "H26"],
    code_first_codes: ["H25", "H26", "H27", "H28"],
    pattern_keywords: ["diabetic cataract", "diabetes with cataract"],
  },
  {
    rule_id: "SEPSIS_UNDERLYING_INFECTION",
    description_en: "For sepsis (A41.- / R65.2-), code FIRST the underlying infection (e.g. J18.9 pneumonia, N39.0 UTI, K35.- appendicitis, T81.4- postprocedural infection/sepsis, L97.- infected chronic skin ulcer), then the sepsis code.",
    description_ar: "في حالات تسمم الدم (A41.- / R65.2-)، يُرمز سبب العدوى الأساسي أولاً (مثل J18.9 لالتهاب رئوي أو N39.0 لعدوى بولية أو T81.4- لعدوى ما بعد الإجراء أو L97.- لقرحة جلدية مزمنة مصابة)، ثم يُضاف رمز تسمم الدم.",
    trigger_codes: ["A41", "A40", "R65.2"],
    companion_codes: ["J18", "N39.0", "K35", "K80", "K57", "K63.3", "L03", "M86", "T81.4", "L97", "L89", "L98"],
    code_first_codes: ["J18", "N39.0", "K35", "K80", "K57", "K63.3", "L03", "M86", "T81.4", "N10", "J15", "K65", "L97", "L89", "L98"],
    pattern_keywords: ["sepsis", "septicemia", "septic shock", "bacteremia", "septicemia due to"],
  },
  {
    rule_id: "ANEMIA_IN_NEOPLASTIC_DISEASE",
    description_en: "Anemia in neoplastic disease (D63.0) requires the underlying neoplasm (C00-D49 or D37-D48) coded first.",
    description_ar: "فقر الدم بسبب ورم (D63.0) يتطلب رمز الورم الأساسي (C00-D49) ويُرمز أولاً.",
    trigger_codes: ["D63.0"],
    companion_codes: ["C", "D37", "D38", "D39", "D4"],
    code_first_codes: ["C0", "C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9", "D37", "D38", "D39"],
    pattern_keywords: ["anemia due to cancer", "anemia in neoplasm", "anemia due to malignancy", "anemia of malignancy"],
  },
  {
    rule_id: "ANEMIA_IN_CKD",
    description_en: "Anemia in CKD (D63.1) requires the underlying CKD stage code (N18.-) coded first.",
    description_ar: "فقر الدم بسبب القصور الكلوي المزمن (D63.1) يتطلب رمز مرحلة القصور الكلوي N18.- ويُرمز أولاً.",
    trigger_codes: ["D63.1"],
    companion_codes: ["N18.9", "N18.6", "N18.5", "N18.4", "N18.32", "N18.31", "N18.3", "N18.2"],
    code_first_codes: ["N18"],
    pattern_keywords: ["anemia of ckd", "anemia of chronic kidney", "anemia due to ckd", "renal anemia", "anemia associated with chronic kidney"],
  },
  {
    rule_id: "SEQUELAE_CVA_CODE_FIRST",
    description_en: "Sequelae of cerebrovascular disease (I69.-) require the underlying cerebrovascular condition (I60-I67, e.g. old infarct I63.-) coded first.",
  ...
  {
    rule_id: "GLOMERULOPATHY_IN_SYSTEMIC_DISEASE",
    description_en: "Glomerular disorders in systemic diseases (N08.-) require the underlying systemic condition (e.g. lupus M32.-, diabetes E10-E13, hypertension I10-I15) coded first.",
    description_ar: "اضطرابات الكبيبات في الأمراض الجهازية (N08.-) تتطلب رمز المرض الجهازي الأساسي (مثل الذئبة M32.- أو السكري E10-E13) ويُرمز أولاً.",
    trigger_codes: ["N08"],
    companion_codes: ["M32", "E10", "E11", "I12", "I10"],
    code_first_codes: ["M32", "E10", "E11", "E13", "I12", "I10"],
    pattern_keywords: ["lupus nephritis", "glomerulopathy", "glomerulonephritis in"],
  },
];
