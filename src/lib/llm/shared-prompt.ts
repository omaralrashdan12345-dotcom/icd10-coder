...

{
  ...
  "secondary_icd10": [ /* chronic comorbidities affecting care, same shape, may be empty */ ],
  "tertiary_icd10":  [ /* external causes W/X/Y, acute complications, symptoms, same shape, may be empty */ ],
  "summary": string,
  "entities_extracted": [
...
2. Secondary = chronic comorbidities that affect the treatment plan (diabetes, HTN, CKD, COPD, etc.).
3. Tertiary = external cause codes (V/W/X/Y), acute complications of the primary, accompanying symptoms, or supporting findings.
...
- The external cause code (V/W/X/Y) MUST be reported and goes in TERTIARY (e.g. W55.03XA = other contact with cat, W19.XXXA = unspecified fall, W54.0XXA = bitten by dog, V89.2XXA = unspecified MVA — all initial encounter). NEVER output an external cause (V/W/X/Y) as Primary — official ICD-10-CM guidelines forbid that.
...
- For an injury with no documented mechanism, use W19.XXXA (unspecified fall) with the matching 7th char.
...
- Acuity: distinguish acute vs chronic vs acute-on-chronic.
...
- Diabetes with complication -> code the complication first if it is the reason for the visit (e.g. foot ulcer -> E11.621 first, then L97.4).
