...
async function main() {
  ...
  await runCase(
    "S4 — type-unspecified stroke -> I69.3 family (I64.9 + I69.4- retired from the classification)",
    "History of stroke 2 years ago with memory problems.",
    (r) => {
      check("primary I63.9 (stroke NOS -> cerebral infarction, unspecified)", r.primary_icd10.code === "I63.9", r.primary_icd10.code);
      check("I69.311 memory deficit present", r.secondary_icd10.some((c) => c.code === "I69.311"), r.secondary_icd10.map((c) => c.code).join(","));
  ...
  await runCase("S5 — post-stroke ataxia", "Post-stroke ataxia, evaluated in clinic.", (r) => {
    check("primary I63.9", r.primary_icd10.code === "I63.9", r.primary_icd10.code);
    check("I69.393 ataxia present", r.secondary_icd10.some((c) => c.code === "I69.393"), r.secondary_icd10.map((c) => c.code).join(","));
    sawI69Case = true;
  });
  ...
  await runCase("P1 — accidental acetaminophen overdose", "Admitted after accidental overdose of Tylenol.", (r, issues) => {
    ...
    check("NO X/Y poisoning external cause (v0.8.2: T-code intent is the ext-cause coding)", !r.tertiary_icd10.some((c) => /^[XY]/.test(c.code)), r.tertiary_icd10.map((c) => c.code).join(","));
  ...
  await runCase("P2 — aspirin suicide attempt", "Suicide attempt: ingested an unknown quantity of aspirin.", (r) => {
    ...
    check("NO X/Y poisoning external cause", !r.tertiary_icd10.some((c) => /^[XY]/.test(c.code)), r.tertiary_icd10.map((c) => c.code).join(","));
  });
